...
# Run CAR model
initialize_model(name = model_name, data = state_list, adjacency = adj, model = "ucar")
run_sampler(name = model_name)
output <- load_samples(name = model_name, burn = 2000)
medians <- get_medians(output) * 100000

# Save results
save(output, medians, file = here("results", "CAR", paste0(model_name, "_results.Rdata")))
...
# Save plot
ggsave(
  filename = here("results", "CAR", paste0(model_name, "_map.png")),
